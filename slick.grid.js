...
(function ($) {
    ...
    function SlickGrid(container, data, columnDefs, options) {
        ...
        function createColumnHeaders() {
            ...
            for (var i = 0; i < columns.length; i++) {
                ...
                var header = $("<div class='ui-state-default slick-header-column' />")
                        ...
                        .width(oneColumn.width - headerColumnWidthDiff)
                        .attr("id", "" + uid + oneColumn.id)
                        .attr("title", oneColumn.toolTip || "")
                        .data("column", oneColumn)
                        .addClass(oneColumn.headerCssClass || "")
                        .appendTo($headers);

                if (options.enableColumnReorder || oneColumn.sortable) {
                    header
                        .on('mouseenter', onMouseEnter)
                        .on('mouseleave', onMouseLeave);
                }

                if (oneColumn.sortable) {
        ...
        function destroy() {
            ...
            if (options.enableColumnReorder && $header.sortable)
                $headers.filter(":ui-sortable").sortable("destroy");
        ...
        function appendRowHtml(stringArray, row, range, dataLength) {
            ...
            stringArray.push("<div class='ui-widget-content " + rowCss + "' style='top:" + getRowTop(row) + "px'>"); //(options.rowHeight * row - offset) - Possible instead of getRowTop(row)
        ...
        function getVisibleRange(viewportTop, viewportLeft) {
            ...
            return {
                top: getRowFromPosition(viewportTop), //Math.floor((viewportTop + offset) / options.rowHeight),
                bottom: getRowFromPosition(viewportTop + viewportH) + 1, //Math.ceil((viewportTop + offset + viewportH) / options.rowHeight),
                leftPx: viewportLeft,
        ...
        function updateRowPositions() {
            for (var row in rowsCache) {
                rowsCache[row].rowNode.style.top = getRowTop(row) + "px"; //rowsCache[row].rowNode.style.top = (row * options.rowHeight - offset) + "px";
            }
        }
        ...
        function getCellFromPoint(x, y) {
            var row = getRowFromPosition(y); //Math.floor((y + offset) / options.rowHeight);
            var cell = 0;
        ...
        function getCellNodeBox(row, cell) {
            ...
            var y1 = getRowTop(row); //row * options.rowHeight - offset;
        ...
        function gotoCell(row, cell, forceEdit) {
            ...
            scrollRowIntoView(row, false);
            scrollCellIntoView(row, cell, false);
            var edit = forceEdit || (row === getDataLength()) || options.autoEdit;
            setActiveCellInternal(getCellNode(row, cell), edit); // if selecting the 'add new' row, start editing right away

            if (!currentEditor) // if no editor was created, set the focus back on the grid
                setFocus();
        }
            ...
            if (currentEditor) {
                if (currentEditor.isValueChanged()) {
                    ...
                    if (validationResults.valid) {
                        ...
                        //TODO: Check if this goes here: $(activeCellNode).stop(true, true).effect("highlight", { color: "red" }, 300);
